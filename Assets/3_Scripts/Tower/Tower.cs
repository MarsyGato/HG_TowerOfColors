...
public class Tower : MonoBehaviour
{
    ...
    IEnumerator BuildTowerSequence()
    {
        ...
        for (int y = 0; y < FloorCount; y++)
        {
            ...
            for (int i = 0; i < TileCountPerFloor; i++)
            {
                ...
                TowerTile tileInstance = PoolManager.Create(Random.value > SpecialTileChance ? TilePrefab.gameObject : SpecialTilePrefabs[Random.Range(0, SpecialTilePrefabs.Length)].gameObject).GetComponent<TowerTile>();
                tileInstance.ResetConnectedTiles();
                tileInstance.transform.position = position;
                tileInstance.transform.rotation = direction * TilePrefab.transform.rotation;
    ...
    {
        StartCoroutine(BuildTowerSequence());
    }
	...
	private void OnDestroy()
    {
        ...
        if (Application.isPlaying)
        {
            PoolManager.DisableAllByType(TilePrefab.name);

            foreach (TowerTile item in SpecialTilePrefabs)
            {
                    PoolManager.DisableAllByType(item.name);
            }
        }
    }
