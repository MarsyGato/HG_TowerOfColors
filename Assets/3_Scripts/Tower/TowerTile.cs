...
public class TowerTile : MonoBehaviour
{
    ...
    [SerializeField] public int _colorindex =  0;
        ...
        if (Active)
        {
            if (Time.time > nextCheckTime && rigidbody.velocity.y < -1)
            {
                ...
                if (!Physics.Raycast(rigidbody.worldCenterOfMass, Vector3.down, rigidbody.worldCenterOfMass.y + 1, 1 << 9))
                {
                    ...
                    if (MissionManager.Instance != null && RemoteConfig.MISSIONS_ENABLED)
                        MissionManager.MissionUpdate(this.GetType().ToString() + "_EXPLODE_" + TileColorManager.Instance.GetUnshuffledIndexes(ColorIndex), 1);
                }
            }
        }
        else if (!freezed)
    ...
    public void ResetConnectedTiles()
	{
    ...
    public void SetColorIndex(int colorIndex)
    {
        ...
        _colorindex = TileColorManager.Instance.GetUnshuffledIndexes(ColorIndex);
